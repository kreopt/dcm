...
void dcm::ipc::shm::server::open(const std::string &_name) {
...
void dcm::ipc::shm::server::close(const std::string &_name) {
    maps_.erase(_name);
}

void dcm::ipc::shm::server::send(const std::string &_name, const dcm::buffer &_data) {
    try {
